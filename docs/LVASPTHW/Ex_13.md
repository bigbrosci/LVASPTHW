...
* 除了使用gamma点，我们还可以使用其他的数值，比如2 2 2，3 3 3， 3 3 1 等，数值越大，计算量也就越大。具体的要根据你自己的体系进行测试，还要结合组里的计算能力来确定，这个我们后面会介绍；
...
ex01  ex02  ex03
qli@bigbrosci 0.01 % mkdir ex04 && cd ex04
qli@bigbrosci 0.01 % for i in {1..6}; do cp ../ex03/0.01/ ${i}${i}${i} ; done 
qli@bigbrosci 0.01 % ls 
111  222  333  444  555  666
qli@bigbrosci 0.01 % cat 333/KPOINTS  -n 
     1	K-POINTS  
     2	 0  
     3	Gamma
     4	1 1 1
     5	0 0 0 
qli@bigbrosci 0.01 % for i in {1..6}; do sed -i '4s/1 1 1/$i $i $i/g' $i/KPOINTS ; done 
